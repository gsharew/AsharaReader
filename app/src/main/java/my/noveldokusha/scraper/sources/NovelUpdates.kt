 ...
 */
class NovelUpdates(private val networkClient: NetworkClient) : SourceInterface.Catalog {
    ...
    override suspend fun getChapterList(bookUrl: String): Response<List<ChapterMetadata>> = withContext(Dispatchers.Default) {
        tryConnect {
    ...
    override suspend fun getCatalogList(index: Int): Response<PagedList<BookMetadata>> = withContext(Dispatchers.Default) {
        tryFlatConnect {
            val page = index + 1
